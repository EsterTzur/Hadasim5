...
def split_by_day(df):
    ...
    for date in unique_sorted_dates:
        ...
        day = df[(df['date'] == date)].drop(columns=['date']) #daily df of full date with value
        if csv:
...
def merge_hourly_results(hourly_files, output_file):
    # united all files to one df
    if csv:
        merged_df = pd.concat([pd.read_csv(file) for file in hourly_files])
    else:
        merged_df = pd.concat([pd.read_parquet(file) for file in hourly_files])
    merged_df['start time'] = pd.to_datetime(merged_df['start time'])#fix date format
