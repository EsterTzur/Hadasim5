...
def errors_in_small_file(small_file):
    ...
    with open(small_file, 'r', encoding='utf-8') as small:
        for error in small:
            # increase by 1 the count of the error that in error
            small_count[error.strip()] += 1
    return small_count


def count_merge_files(number_of_files):
...
def find_common_n(N, log_file):
    ...
    number_of_files = split_file(log_file)
    # count errors of each file and then merge them
    error_count = count_merge_files(number_of_files)
...
if __name__ == "__main__":
    ...
    result = find_common_n(N, log_file)
    # print the resultS
